...
typename std::enable_if<Owned, unique_ptr<MockTester>>::type getTester() {
  return coral::make_unique<MockTester>();
}
...
class GenericFilterTest: public testing::Test {
 public:
  ...
  typename std::enable_if<Owned, unique_ptr<FilterT>>::type getFilter() {
    return coral::make_unique<FilterT>();
  }
...
TEST_F(OwnedGenericFilterTest, single_elem_chain) {
  auto filterUnique = coral::make_unique<TestFilter<true>>();
...
TEST_F(OwnedGenericFilterTest, multi_elem_chain) {
  auto f1 = coral::make_unique<TestFilter<true>>();
  auto f2 = coral::make_unique<TestFilter<true>>();
  auto f3 = coral::make_unique<TestFilter<true>>();
...
TEST_F(OwnedGenericFilterTest, multi_elem_multi_add) {
  ...
  for (unsigned i = 0; i < 10; ++i) {
    auto filter = coral::make_unique<TestFilter<true>>();
...
TEST_F(OwnedGenericFilterTest, wants) {
  auto f1 = coral::make_unique<TestFilter<true>>();
  auto f2 = coral::make_unique<TestFilterNoCallback<true>>();
  auto f3 = coral::make_unique<TestFilterNoCall<true>>();
  auto f4 = coral::make_unique<TestFilterNoCallbackNoCall<true>>();
  TestFilter<true>* fp1 = f1.get();
  TestFilter<true>* fp2 = f2.get();
  TestFilter<true>* fp3 = f3.get();
...
TEST_F(OwnedGenericFilterTest, wants_multi_add) {
  auto f1 = coral::make_unique<TestFilterNoCallback<true>>();
  auto f2 = coral::make_unique<TestFilterNoCall<true>>();
  TestFilter<true>* fp1 = f1.get();
  TestFilter<true>* fp2 = f2.get();
...
TEST_F(OwnedGenericFilterTest, change_callback) {
  ...
  auto f = coral::make_unique<TestFilterNoCallback<true>>();
...
TEST_F(OwnedGenericFilterTest, delete_chain) {
  ...
  for (unsigned i = 0; i < NUM_FILTERS; ++i) {
    chain().addFilters(
      coral::make_unique<TestFilterOddDeleteOn<true>>(&deletions));
  }
  chain_.reset();
